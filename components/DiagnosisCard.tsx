...
import { 
  ...
  Heart, Leaf, Save, Check, Loader2, Anchor, Zap, Activity, Sparkles, Scale, Copy, Share2, Download
} from 'lucide-react';
...
${diagnosis.lifestyleAdvice}
  ...
  const handleDownloadTXT = () => {
    const textContent = `==================================================
TCM CLINICAL DIAGNOSIS REPORT
==================================================
Date generated    : ${new Date().toLocaleString('id-ID')}
Patient Name     : ${patientContext?.patientName || "Pasien Anonim"}
Age / Sex        : ${patientContext?.age || "-"} / ${patientContext?.sex || "-"}
Complaint        : ${patientContext?.complaint || "Konsultasi Chat"}

DIAGNOSIS INTAKE DETAILS:
- Symptoms Checked: ${patientContext?.symptoms || ""} ${patientContext?.selectedSymptoms?.join(', ') || ''}
- Tongue Body    : ${patientContext?.tongue?.body_color || 'Normal'}
- Tongue Coat    : ${patientContext?.tongue?.coating_color || 'Normal'} ${patientContext?.tongue?.coating_quality ? `(${patientContext.tongue.coating_quality})` : ''}
- Tongue Features: ${patientContext?.tongue?.special_features?.join(', ') || 'None'}
- Pulse Qualities: ${patientContext?.pulse?.qualities?.join(', ') || 'None'}
${patientContext?.notes ? `- Additional Observ: ${patientContext.notes}\n` : ''}
==================================================
DIAGNOSIS ASSESSMENT (CDSS RESULTS):
==================================================
Pattern Name / ID: ${diagnosis.patternId}
Explanation      : ${diagnosis.explanation}

BEN (Root / Akar):
${diagnosis.differentiation?.ben?.map(item => `  - ${item.label}: ${item.value} (${item.score}% match)`).join('\n') || '  N/A'}

BIAO (Branch / Cabang):
${diagnosis.differentiation?.biao?.map(item => `  - ${item.label}: ${item.value} (${item.score}% match)`).join('\n') || '  N/A'}

==================================================
PROPOSED ACUPUNCTURE FORMULA:
==================================================
Recommended Points:
${diagnosis.recommendedPoints?.map(p => `  - [${p.code}] ${p.description}`).join('\n') || '  N/A'}
...
==================================================
CLINICAL STRATEGIES & LIFESTYLES:
==================================================
Herbal Formula   : ${diagnosis.classical_prescription || diagnosis.herbal_recommendation?.formula_name || 'Individual Strategy'}
Lifestyle Advice : ${diagnosis.lifestyleAdvice}

--------------------------------------------------
TCM WuXing Pro Clinical Decision Support System
==================================================`;
    ...
    link.href = url;
    link.download = `TCM_Patient_Report_${diagnosis.patternId.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.txt`;
  ...
  return (
    <div className="mt-6 space-y-4 animate-fade-in print:text-black">
      ...
      <div ref={cardRef} className="bg-white border border-purple-100 rounded-3xl p-6 shadow-2xl relative overflow-hidden print:bg-white print:border-slate-300">
        ...
        {/* Bottom Actions Row */}
        <div className="mt-8 pt-6 border-t border-purple-100 flex flex-wrap gap-3 justify-end items-center print:hidden" data-html2canvas-ignore="true">
           <span className="text-[10px] font-black uppercase text-purple-400 tracking-widest mr-auto">
             Laporan Selesai • Ekspor Data
           </span>
           <button 
             onClick={handleCopy}
             className="px-4 py-2.5 rounded-xl border border-purple-200 bg-purple-50 text-purple-700 hover:text-white hover:bg-purple-600 hover:border-purple-600 text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center gap-2"
           >
             <Copy className="w-3.5 h-3.5" /> Salin Teks
           </button>
           <button 
             onClick={handleDownloadTXT}
             className="px-4 py-2.5 rounded-xl border border-teal-200 bg-teal-50/50 text-teal-700 hover:text-white hover:bg-teal-600 hover:border-teal-600 text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center gap-2"
           >
           ...
           <button 
             ...
             className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2 shadow-lg shadow-purple-900/10"
           >
             {isExporting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FileDown className="w-3.5 h-3.5" />} Simpan PDF
           </button>
        </div>
      </div>
    </div>
