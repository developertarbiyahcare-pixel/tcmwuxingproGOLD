    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-purple-950/70 backdrop-blur-md p-4 animate-fade-in">
      <div className="bg-white border border-purple-100 w-full max-w-2xl rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-10">
            ...
            {/* Clinical Notes & Observations Section V */}
            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.3em] border-b border-purple-100 pb-2 flex items-center gap-2">
                <FileText className="w-4 h-4" /> V. Clinical Notes & Observations
              </h3>
              <div>
                 <label className="block text-[10px] font-black text-purple-400 uppercase mb-2 ml-1 tracking-widest">Additional Observations / Notes</label>
                 <textarea 
                    className="w-full bg-purple-50/50 border border-purple-200 rounded-2xl px-5 py-4 text-sm text-purple-900 focus:border-tcm-primary focus:bg-white outline-none h-28 resize-none shadow-inner transition-all"
                    value={formData.notes}
                    onChange={e => setFormData({...formData, notes: e.target.value})}
                    placeholder="Record any specific symptoms, pulse anomalies, client conditions, or general observations not included in standard checklists..."
                 />
              </div>
            </div>
