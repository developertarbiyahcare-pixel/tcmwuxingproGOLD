...
import { MapPin, Info, Zap, Search, Layers, Sparkles } from 'lucide-react';
...
import { MeridianMapOverlay } from './MeridianMapOverlay';
...
const AcupuncturePointsPanel: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Interactive Mode Swapper Bar */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-purple-50 p-2.5 rounded-3xl border border-purple-100">
        <div>
          <span className="text-[10px] text-purple-600 font-extrabold uppercase tracking-widest block">Reference Workspace Mode</span>
          <p className="text-[11px] text-purple-950 font-bold leading-none mt-0.5">Toggle anatomical diagnostics or manual databases</p>
        </div>
        <div className="flex bg-white/80 p-0.5. rounded-2xl border border-purple-200/50 w-full sm:w-auto overflow-hidden">
          <button
            onClick={() => setViewMode('MAP')}
            className={`flex-1 sm:flex-initial px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all ${
              viewMode === 'MAP'
                ? 'bg-gradient-to-r from-teal-500 to-emerald-600 text-white shadow-md'
                : 'text-purple-600 hover:text-purple-800'
            }`}
          >
            <MapPin className="w-3.5 h-3.5" /> Meridian Map Overlay
          </button>
          <button
          ...
          >
            <Layers className="w-3.5 h-3.5" /> Clinical Point Cards
          </button>
        </div>
      </div>
      ...
      {viewMode === 'MAP' ? (
        <div className="animate-fade-in">
          <MeridianMapOverlay points={allPoints} />
        </div>
      ) : (
        <div className="bg-white rounded-3xl border border-purple-100 shadow-xl overflow-hidden animate-fade-in">
      ...
      <div className="bg-amber-50 border border-amber-105 p-6 rounded-3xl">
