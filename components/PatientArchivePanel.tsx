  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        console.warn("Failed to load settings in Archive Panel:", err);
      }
    };
    loadSettings();
      ...
      <div className="flex-1 overflow-y-auto bg-white p-6 md:p-10 relative scrollbar-hide">
         {!selectedPatient ? (
           ...
           <div ref={pdfExportRef} className="max-w-4xl mx-auto space-y-8 animate-slide-in-right p-4 bg-white rounded-2xl">
              {/* CLINICAL LETTERHEAD */}
              <div className="flex flex-col sm:flex-row items-center justify-between border-b-2 border-purple-800 pb-4 mb-6 gap-4">
                 <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center border border-purple-200">
                       <Activity className="w-8 h-8 text-tcm-primary" />
                    </div>
                    <div>
                       <h2 className="text-xl font-black text-purple-900 tracking-tight uppercase">
                          {settings?.clinicName || 'TCM WU XING CLINIC & CARE CENTER'}
                       </h2>
                       <p className="text-xs text-purple-500 font-semibold leading-none">
                          {settings?.clinicAddress || 'Layanan Kedokteran Timur & Akupunktur Profesional'}
                        </p>
                  ...
                  <div className="text-right shrink-0 sm:text-right text-center">
                     <span className="text-[10px] font-black text-purple-500 uppercase tracking-widest block">DOKUMEN REKAM MEDIS & DIAGNOSIS</span>
                     <span className="text-[9px] font-mono text-purple-400">ID: {selectedPatient.id}</span>
                  </div>
               </div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start border-b border-purple-100 pb-6 gap-4">
                 ...
                 <div className="flex flex-wrap items-center gap-3 w-full md:w-auto" data-html2canvas-ignore="true">
                    <button
                        ...
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-white hover:bg-purple-50 text-purple-600 border border-purple-200 rounded-xl font-bold shadow-sm transition-all active:scale-95 disabled:opacity-50 cursor-pointer"
                    >
                        {isExporting ? (
                    ...
                    <button 
                        ...
                        className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold shadow-sm transition-all border border-purple-500 cursor-pointer"
                    >
              ...
              {/* BEN & BIAO DIFFERENTIATION */}
              {selectedPatient.diagnosis.differentiation && (selectedPatient.diagnosis.differentiation.ben || selectedPatient.diagnosis.differentiation.biao) && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {selectedPatient.diagnosis.differentiation.ben && selectedPatient.diagnosis.differentiation.ben.length > 0 && (
                       <div className="bg-purple-50 border border-purple-200 p-5 rounded-2xl shadow-sm">
                          <h3 className="text-xs font-black text-purple-800 uppercase tracking-widest mb-3 flex items-center gap-2">
                             <TrendingUp className="w-4 h-4 text-tcm-primary" /> Ben (Akar / Root Cause)
                          </h3>
                          <div className="bg-white rounded-xl border border-purple-100 overflow-hidden divide-y divide-purple-100 shadow-sm">
                             {selectedPatient.diagnosis.differentiation.ben.map((item, i) => (
                                <div key={i} className="p-3 text-xs flex justify-between items-center bg-purple-50/20">
                                   <span className="font-bold text-purple-900 w-1/3">{item.label}</span>
                                   <span className="text-purple-700 italic flex-1 text-right">{item.value}</span>
                                </div>
                             ))}
                          </div>
                       </div>
                    )}
                    {selectedPatient.diagnosis.differentiation.biao && selectedPatient.diagnosis.differentiation.biao.length > 0 && (
                       <div className="bg-purple-50 border border-purple-200 p-5 rounded-2xl shadow-sm">
                          <h3 className="text-xs font-black text-purple-800 uppercase tracking-widest mb-3 flex items-center gap-2">
                             <Activity className="w-4 h-4 text-purple-600" /> Biao (Manifestasi / Branch Symptoms)
                          </h3>
                          <div className="bg-white rounded-xl border border-purple-100 overflow-hidden divide-y divide-purple-100 shadow-sm">
                             {selectedPatient.diagnosis.differentiation.biao.map((item, i) => (
                                <div key={i} className="p-3 text-xs flex justify-between items-center bg-purple-50/20">
                                   <span className="font-bold text-purple-900 w-1/3">{item.label}</span>
                                   <span className="text-purple-700 italic flex-1 text-right">{item.value}</span>
                                </div>
              ...
              {/* TREATMENT PRINCIPLE (PRINSIP TERAPI) */}
              {selectedPatient.diagnosis.treatment_principle && selectedPatient.diagnosis.treatment_principle.length > 0 && (
                 <div className="bg-purple-50 border border-purple-200 p-6 rounded-2xl shadow-sm">
                    <h3 className="text-sm font-black text-purple-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                       <Pill className="w-4 h-4 text-purple-600" /> Prinsip Terapi (Treatment Principle)
                    </h3>
                    <div className="flex flex-wrap gap-2">
                       {selectedPatient.diagnosis.treatment_principle.map((principle, i) => (
                          <span key={i} className="px-3 py-1.5 bg-white border border-purple-200 text-purple-700 text-xs font-bold rounded-xl shadow-sm">
                             {principle}
                          </span>
                       ))}
                    </div>
              ...
              {/* ACUPUNCTURE POINTS */}
              {((selectedPatient.diagnosis.recommendedPoints && selectedPatient.diagnosis.recommendedPoints.length > 0) || 
                (selectedPatient.diagnosis.masterTungPoints && selectedPatient.diagnosis.masterTungPoints.length > 0)) && (
                 <div className="bg-purple-50 border border-purple-200 p-6 rounded-2xl shadow-sm">
                    <h3 className="text-sm font-black text-purple-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                       <MapPin className="w-4 h-4 text-purple-600" /> Resep Akupunktur & Titik Terapi
                    </h3>
                    <div className="space-y-4">
                       {selectedPatient.diagnosis.recommendedPoints && selectedPatient.diagnosis.recommendedPoints.length > 0 && (
                          <div>
                             <span className="text-[10px] font-black text-purple-400 uppercase tracking-wider block mb-2">Titik Standar Klasis (TCM Points)</span>
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {selectedPatient.diagnosis.recommendedPoints.map((pt, i) => (
                                   <div key={i} className="bg-white border border-purple-100 p-3 rounded-xl flex gap-3 items-start shadow-sm">
                                      <span className="w-8 h-8 rounded-lg bg-purple-100 border border-purple-300 flex items-center justify-center shrink-0 font-black text-purple-800 text-xs">{pt.code}</span>
                                      <div className="text-xs text-purple-700 leading-snug">
                                         <p className="font-bold text-purple-900">{pt.code}</p>
                                         <p className="text-[10px] text-purple-500 font-medium">{pt.description}</p>
                                      </div>
                                   </div>
                                ))}
                             </div>
                       ...
                       {selectedPatient.diagnosis.masterTungPoints && selectedPatient.diagnosis.masterTungPoints.length > 0 && (
                          <div className="border-t border-purple-100 pt-4">
                             <span className="text-[10px] font-black text-amber-600 uppercase tracking-wider block mb-2">Titik Master Tung (Tung's Acupuncture)</span>
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {selectedPatient.diagnosis.masterTungPoints.map((pt, i) => (
                                   <div key={i} className="bg-amber-50/50 border border-amber-200 p-3 rounded-xl flex gap-3 items-start shadow-sm">
                                      <span className="w-8 h-8 rounded-lg bg-amber-100 border border-amber-300 flex items-center justify-center shrink-0 font-black text-amber-800 text-xs">{pt.code}</span>
                                      <div className="text-xs text-amber-900 leading-snug">
                                         <p className="font-bold text-amber-950">{pt.code}</p>
                                         <p className="text-[10px] text-amber-700 font-medium">{pt.description}</p>
                                      </div>
                                   </div>
                                ))}
                             </div>
              ...
              {/* HERBAL PRESCRIPTIONS & ADVICE */}
              {((selectedPatient.diagnosis.herbal_recommendation && selectedPatient.diagnosis.herbal_recommendation.formula_name) || 
                selectedPatient.diagnosis.lifestyleAdvice || 
                selectedPatient.diagnosis.classical_prescription) && (
                 <div className="bg-purple-50 border border-purple-200 p-6 rounded-2xl shadow-sm">
                    <h3 className="text-sm font-black text-purple-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                       <FileText className="w-4 h-4 text-purple-600" /> Herbal & Saran Gaya Hidup
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div>
                          <span className="text-[10px] font-black text-purple-400 uppercase tracking-wider block mb-2">Formula Herbal Dokter</span>
                          <div className="bg-white border border-purple-100 p-4 rounded-xl min-h-[100px] flex flex-col justify-between shadow-sm">
                             <div>
                                <p className="text-sm font-black text-purple-900 uppercase">
                                   {selectedPatient.diagnosis.herbal_recommendation?.formula_name || selectedPatient.diagnosis.classical_prescription || 'Formula Penyeimbang Wu Xing'}
                                </p>
                                {selectedPatient.diagnosis.herbal_recommendation?.chief && selectedPatient.diagnosis.herbal_recommendation.chief.length > 0 && (
                                   <div className="mt-2 flex flex-wrap gap-1.5">
                                      {selectedPatient.diagnosis.herbal_recommendation.chief.map(herb => (
                                         <span key={herb} className="px-2 py-0.5 bg-emerald-50 text-emerald-800 border border-emerald-200 font-bold text-[9px] rounded uppercase">
                                            {herb} (Chief)
                                         </span>
                                      ))}
                                   </div>
                       ...
                       <div>
                          <span className="text-[10px] font-black text-purple-400 uppercase tracking-wider block mb-2">Edukasi & Gaya Hidup</span>
                          <div className="bg-white border border-purple-100 p-4 rounded-xl min-h-[100px] flex flex-col justify-center shadow-sm">
                             <p className="text-xs text-purple-700 leading-relaxed italic">
                                "{selectedPatient.diagnosis.lifestyleAdvice || 'Lakukan olahraga teratur, jaga kesehatan mental, dan hindari konsumsi makanan dingin.'}"
                             </p>
                          </div>
                       </div>
                    </div>
              ...
              {(selectedPatient.diagnosis.obesity_indication || selectedPatient.diagnosis.beauty_acupuncture) && (
                 <div className="bg-purple-50 border border-purple-200 p-6 rounded-2xl shadow-sm">
                    <h3 className="text-sm font-black text-purple-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                       <Activity className="w-4 h-4 text-purple-600" /> Indikasi Khusus Terapi
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       {selectedPatient.diagnosis.obesity_indication && (
                          <div className="bg-white p-4 rounded-xl border border-purple-100 shadow-sm">
                             <span className="text-[9px] font-black text-purple-400 uppercase tracking-wider block mb-1">Manajemen Berat Badan</span>
                             <p className="text-xs text-purple-700 font-medium">{selectedPatient.diagnosis.obesity_indication}</p>
                          </div>
                       )}
                       {selectedPatient.diagnosis.beauty_acupuncture && (
                          <div className="bg-white p-4 rounded-xl border border-purple-100 shadow-sm">
                             <span className="text-[9px] font-black text-purple-400 uppercase tracking-wider block mb-1">Akupunktur Kecantikan</span>
                             <p className="text-xs text-purple-700 font-medium">{selectedPatient.diagnosis.beauty_acupuncture}</p>
                          </div>
                       )}
                    </div>
                 </div>
              ...
              {/* CURRENT MEDICATIONS & FOLLOW UP PLAN */}
              {(selectedPatient.medications || selectedPatient.followUpDate) && (
                 <div className="bg-purple-50 border border-purple-200 p-6 rounded-2xl shadow-sm">
                    <h3 className="text-sm font-black text-purple-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                       <Clock className="w-4 h-4 text-purple-600" /> Rencana Tindak Lanjut & Obat Aktif
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-semibold">
                       {selectedPatient.medications && (
                          <div className="bg-white p-4 rounded-xl border border-purple-100 shadow-sm">
                             <span className="text-[9px] font-black text-purple-400 uppercase tracking-wider block mb-1">Obat / Suplemen Saat Ini</span>
                             <p className="text-purple-800 font-medium whitespace-pre-line">{selectedPatient.medications}</p>
                          </div>
                       )}
                       {selectedPatient.followUpDate && (
                          <div className="bg-white p-4 rounded-xl border border-purple-100 flex flex-col justify-center shadow-sm">
                             <span className="text-[9px] font-black text-purple-400 uppercase tracking-wider block mb-1">Rencana Kunjungan Kembali</span>
                             <p className="text-lg font-black text-purple-900">
                                {new Date(selectedPatient.followUpDate).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                             </p>
                          </div>
                       )}
                    </div>
